...
- **Turbopack stale cache**: after rapid sequential edits Turbopack may report "name defined multiple times" even when the file is correct on disk. Fix: kill the dev server, `Remove-Item -Recurse -Force .next`, restart. The page may still render from the initial compile even while errors loop in the console.
...
| 2026-06-03 | Mobile UX: (1) ShopView — checked items sink to collapsed "Got it · N" section at bottom; unchecked items always at top for in-store clarity. Tap "Got it" to expand and uncheck. (2) RecipeViewerModal — `navigator.wakeLock` keeps screen on while recipe is open; released on close. |
| 2026-06-04 | Source URL: `sourceUrl String?` added to `Recipe` model. URL import saves the origin URL; shown as clearable link in RecipeEditorModal and "View original recipe →" link in RecipeViewerModal. |
| 2026-06-04 | First-time onboarding: `OnboardingModal` (4-step welcome wizard, localStorage `fmc_onboarded`) + `TabHint` (per-tab dismissible hint banners, `fmc_hint_<tab>` keys). No new dependencies. |
| 2026-06-06 | Emoji removal + ingredient editing: (1) Removed category emojis everywhere — `CATS.i` field dropped, `catIcon()` deleted; category pills and section headers now text+color only. (2) RecipeEditorModal: inline edit per ingredient row (pencil icon → expand form → Save/Cancel). (3) ShopView: store chip is now a tappable button; "set store" ghost button for unassigned items; bottom-sheet store picker. (4) New `PATCH /api/ingredients` endpoint updates store by name across all user recipes. (5) `updateIngredientStore` Zustand action (optimistic). |
| 2026-06-06 | Bug fixes: (1) URL import now reads `schema.recipeYield` via `parseYield()` for servings instead of relying solely on AI inference — resolves servings always defaulting to 4. (2) RecipeEditorModal `timeRow` gets `flex-wrap: wrap` + `min-width: 120px` per field so Cook doesn't overflow on narrow phones; spinner buttons fully removed. (3) RecipesView `rcInfo`/`rcName`/`rcMeta`/`rcTime` get proper `overflow: hidden` + `text-overflow: ellipsis` to prevent any text from bleeding outside card bounds on mobile. |
| 2026-06-06 | Voice pantry import: `Speak` button in PantryView header opens a voice panel. Browser `SpeechRecognition` API captures speech in real-time; transcript sent to `/api/pantry/voice-import` (Haiku structured output) which returns parsed `{name, qty, unit}` items. User reviews/edits parsed items before bulk-adding. `SpeechRecognition` types declared locally (not in this TS lib version). |
| 2026-06-10 | Stitch-inspired UI Phase 1: (1) Header flipped from dark to light cream (glass/blur, `rgba(253,248,240,0.92)`), centered "Tonight" wordmark in terracotta, 3-col grid layout. (2) WeekView overview dots → horizontal day picker strip (day abbr + date number, today = solid terracotta). (3) Food photo thumbnails (60×60px) in day card rows — uses `recipe.imageUrl` if set, else fetches from `/api/meal-image` (Unsplash proxy, `UNSPLASH_ACCESS_KEY` env var). Gracefully degrades if no key. |
| 2026-06-10 | Stitch-inspired UI Phase 2: WeekView home screen redesigned as day-focused single-day view. Vertical 7-day card list replaced with a `.todaySection` showing Stitch-style horizontal meal cards (88×88 photo + category label + recipe name + derived time + cooked/add action). Day picker navigates the displayed day; heading switches "Today's Menu" ↔ "DayName's Menu". Empty day shows illustration-style dashed card + "Plan this day" CTA. `slotTime()` helper derives human times from slot name. Turbopack stale-cache gotcha: after rapid sequential edits delete `.next` and restart dev server. |
