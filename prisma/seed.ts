...
async function main() {
  ...
  const smoked = await prisma.recipe.findFirst({ where: { name: 'Smoked Salmon Frittata' } });
  const greek  = await prisma.recipe.findFirst({ where: { name: 'Greek Chicken Bowls' } });

  await prisma.recurringMeal.upsert({
    where: { key: 'brunch' },
    update: { recipeId: smoked?.id },
    create: { key: 'brunch', label: 'Brunch', recipeId: smoked?.id },
  });
  await prisma.recurringMeal.upsert({
    where: { key: 'lunch' },
    update: { recipeId: greek?.id },
    create: { key: 'lunch', label: 'Weekday Lunch Prep', recipeId: greek?.id },
  });
